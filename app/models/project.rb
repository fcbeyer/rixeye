class Project < ActiveRecord::Base
  ...
  def parse_xml(log_file,problem,have_new_revision_data)
  	begin
  		next_revision = self.last_revision.nil? ? self.base_revision : self.last_revision + 1
  		response = system "svn log --verbose --username #{Rixeye::Application.config.rixeye_settings['SVN_User']} --password #{Rixeye::Application.config.rixeye_settings['SVN_Password']} #{self.url_path} -r#{next_revision}:HEAD --xml > #{log_file}"
